...
- Validation on one of the inputs
...
# ✔  ++ /example-output/example-2-all-working/index.html
# ✔  ++ /example-output/example-2-all-working/script.js
...
If a user bypasses the `lang` prompt on the command line, an invalid value is
rejected:
...
Currently Example 2 accepts an empty `heading`, or a `heading` that just contains
whitespace characters.

In the plopfile.js change the `heading` input to:
...
Now Example 2 will not accept an invalid `heading`.
